...
var errorInterceptor = (req, next) => {
  ...
  return next(req).pipe(catchError((error) => {
    ...
    if (error.error && error.error.message) {
      ...
      errorMsg = "Sesi\xF3n expirada o credenciales incorrectas. Por favor inicie sesi\xF3n.";
      authService.logout();
    } else if (error.status === 403) {
      errorMsg = "No tiene permisos suficientes para realizar esta acci\xF3n.";
    } else if (error.status === 404) {
      errorMsg = "El recurso solicitado no fue encontrado.";
    } else if (error.status === 0) {
      errorMsg = "No se pudo conectar con el servidor backend (Spring Boot). Verifique que est\xE9 iniciado.";
    }
